...
// NOTE: it is mostly copypasted from https://github.com/rivo/tview/blob/master/dropdown.go ,
// but the original one is really underdeveloped, so as the fastest thing I just copy-pasted
// it and added a few things that I needed:
//
...
func (d *DropDown) GetFieldWidth() int {
	...
	for _, option := range d.options {
		width := tview.TaggedStringWidth(option.Text)
		if width > fieldWidth {
			fieldWidth = width
		}
	}
...
func (d *DropDown) Draw(screen tcell.Screen) {
	...
	maxWidth := 0
	optionWrapWidth := tview.TaggedStringWidth(d.optionPrefix + d.optionSuffix)
	for _, option := range d.options {
	...
	if fieldWidth == 0 {
		...
		if d.currentOption < 0 {
			noSelectionWidth := tview.TaggedStringWidth(d.noSelection)
			if noSelectionWidth > fieldWidth {
				fieldWidth = noSelectionWidth
			}
		} else if d.currentOption < len(d.options) {
			currentOptionWidth := tview.TaggedStringWidth(d.currentOptionPrefix + d.options[d.currentOption].Text + d.currentOptionSuffix)
	...
	if d.open && len(d.prefix) > 0 {
		// Show the prefix.
		currentOptionPrefixWidth := tview.TaggedStringWidth(d.currentOptionPrefix)
		prefixWidth := stringWidth(d.prefix)
		listItemText := d.options[d.list.GetCurrentItem()].Text
		tview.Print(screen, d.currentOptionPrefix, x, y, fieldWidth, tview.AlignLeft, d.fieldTextColor)
		tview.Print(screen, d.prefix, x+currentOptionPrefixWidth, y, fieldWidth-currentOptionPrefixWidth, tview.AlignLeft, d.prefixTextColor)
		if len(d.prefix) < len(listItemText) {
			tview.Print(screen, listItemText[len(d.prefix):]+d.currentOptionSuffix, x+prefixWidth+currentOptionPrefixWidth, y, fieldWidth-prefixWidth-currentOptionPrefixWidth, tview.AlignLeft, d.fieldTextColor)
		}
	} else {
...
func (d *DropDown) openList(setFocus func(tview.Primitive)) {
	d.open = true
...
// from it.
func (d *DropDown) closeList(setFocus func(tview.Primitive)) {
	d.open = false
	if d.list.HasFocus() {
		setFocus(d)
	}
	...
	return d.WrapMouseHandler(func(action tview.MouseAction, event *tcell.EventMouse, setFocus func(p tview.Primitive)) (consumed bool, capture tview.Primitive) {
		...
		case tview.MouseLeftDown:
			...
			if !d.open {
				d.openList(setFocus)
				d.dragging = true
			} else if consumed, _ := d.list.MouseHandler()(tview.MouseLeftClick, event, setFocus); !consumed {
		...
		case tview.MouseMove:
			if d.dragging {
				...
				d.list.MouseHandler()(tview.MouseLeftClick, event, setFocus)
				consumed = true
				capture = d
			}
